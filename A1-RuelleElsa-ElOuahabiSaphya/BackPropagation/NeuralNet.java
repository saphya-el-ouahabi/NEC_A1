...
public class NeuralNet {
  ...
  public void updateWeights() {
	  ...
	  for (int lay=1; lay<L-1 ;lay++) {
		  for (int i=0;i<n[lay];i++) {
			  for (int j=0;j<n[lay-1];j++) {
				  d_w[lay][i][j] = -learningRate*sum_delta[lay][i]*sum_xi[lay-1][j]+alpha*d_w_prev[lay][i][j] ;
  ...
  public void updateThresholds() {
	  ...
	  for (int lay=1; lay<L-1 ;lay++) {
		  for (int i=0;i<n[lay];i++) {
			  d_theta[lay][i] = learningRate*sum_delta[lay][i]+alpha*d_theta_prev[lay][i] ;
  ...
  public static void main(String[] args) {
	...
	Dataset dataset = new Dataset("Normalized_datasets/A1-turbine-norm.txt", 451, 5, 0.85);
    int[] layers = {4, 9, 5, 1};
    NeuralNet nn = new NeuralNet(layers);
    nn.backPropagation("Normalized_datasets/A1-turbine",dataset.getTrainData(), dataset.getTestData(), dataset.getTrainTarget(), dataset.getTestTarget());
    
    System.out.println("\n"+"File : A1-synthetic"+"\n");
	Dataset dataset2 = new Dataset("Normalized_datasets/A1-synthetic_normalized.txt", 1000, 10, 0.80);
    int[] layers2 = {9, 9, 5, 1};
    NeuralNet nn2 = new NeuralNet(layers2);
    nn2.backPropagation("Normalized_datasets/A1-synthetic",dataset2.getTrainData(), dataset2.getTestData(), dataset2.getTrainTarget(), dataset2.getTestTarget());
  	
    System.out.println("\n"+"File : insurance_dataset"+"\n");
	Dataset dataset3 = new Dataset("Normalized_datasets/insurance_dataset_norm.txt", 1193, 7, 0.80);
    int[] layers3 = {6, 9, 5, 1};
    NeuralNet nn3 = new NeuralNet(layers3);
    nn3.backPropagation("Normalized_datasets/insurance_dataset",dataset3.getTrainData(), dataset3.getTestData(), dataset3.getTrainTarget(), dataset3.getTestTarget());
